...
| 구간 스킵 | `F8` / `F9` | `F8`: F→G→헬테 다음 전투 구간 이동 / `F9`: 헬테 직전으로 즉시 이동. 위치뿐 아니라 퀘스트·HUD·적·문 상태도 함께 전환되며 Windows·macOS 릴리스 빌드에서도 사용 가능 |

훈련장과 F/G 구역의 기능은 Player 빌드에서 제거되는 `EditorOnly` 저작 마커에 의존하지 않습니다. 점프 투사체·근접 허수아비·F/G 적 생성과 배경 전환은 빌드에 남는 런타임 앵커 및 현재 퀘스트 상태에서 복구됩니다.
...
- [DEMO 릴리스 노트](Assets/_Project/Docs/RELEASE_DEMO_V1.md) — 배포 구성, 실행 방법과 검증 범위

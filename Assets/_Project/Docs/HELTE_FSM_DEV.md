...
- 헬테 최대 체력: `2500` (2026-08-10 플레이 폴리싱 기준)
- 2페이즈 기준: 최대 체력의 `55%` (`1375`)
- 최종 러시 기준: 최대 체력의 `20%` (`500`)
- 목표 전투 시간: `300초`
- 체력만 높은 전투를 피하기 위해 기존 `6000 → 5000 → 2500`으로 낮추고, 후반부의 이동·회복 템포와 공명 스킬을 함께 계측한다.
...
## 2026-08-10 보스 플레이 폴리싱 패스

적용 씬은 `Assets/Scenes/BossDevelopmentScene.unity`다. 저스트 대시 보상은 이번 범위에서 제외했다.

- 헬테 최대 체력 `5000 → 2500`, 프로메 최대 체력 `100 → 500`
- 기존 제작 UI 스프라이트 `TUTO_UI_BarTrack_v4`를 헬테 체력바 트랙에 연결
- 체력바에 2페이즈(55%)·최종 러시(20%) 마커와 `공격 기회`/`공격 금지` 상태 문구 추가
- 전용 스킬 키 `3`으로 4연속 참격 발동. 공명 충전 조건은 제거하고 10초 쿨다운만 사용
  - 기존 프로메 `Attack01` PNG 시퀀스를 재사용하되, 실제 스프라이트가 바라보는 방향을 발동 순간 잠그고 그 앞에서 축소된 참격 아크를 1타씩 교체 표시
  - 전용 스킬 HUD는 설명·키 문구·공명 게이지 없이 아이콘과 원형 쿨다운 오버레이만 표시
  - 4타 적중 시 `0.075초` 실시간 히트스톱, 접근성 강도 제한을 받는 약한 카메라 흔들림, `SFX_Impact_Heavy` 강조음을 동시에 재생
  - 중간 타격이 빗나가면 연속 참격이 즉시 끊기고, 사용 후 재사용 대기시간은 10초
- 테우스가 보스전 중에만 1초 간격으로 자동 원거리 지원 공격. 시각적 존재감은 높이되 발당 피해를 4로 낮춰 기존 전투 시간에 주는 영향은 제한
  - 착탄 위치에는 풀링된 청백색 스파크가 짧게 확대·페이드되어 명중을 명확하게 표시
- 프로메 첫 사망은 테우스가 체력 50%와 1.5초 무적으로 1회 부활
- 헬테 2페이즈 전환 시 프로메 체력 전부 회복
- 페이크 블링크, 카운터, 자비 후퇴와 최종 러시 상태가 기존 헬테 애니메이션 클립으로 연결되도록 매핑 보정
- 보스 개발 씬의 프로메는 메인 `TutorialScene`과 동일하며, 테우스·헬테 모델 및 헬테 Animator도 메인 씬 기준으로 동기화

씬 적용 자동화 명령은 `boss.polish.apply`다. 보스 개발 씬에서만 실행되며, `dryRun` 검토 후 실제 적용한다.
